...
::ExtraShotEventTable <- {
	function OnGameEvent_player_shoot(params)
	{
		...
		local extrashot = weapon.GetAttribute("extra shot", 1)
		if (extrashot)
		{
...
function ExtraShootThink()
{
	...
	firebullets.SetDistance(weapon.GetAttribute("extra shot range", 1000))
	firebullets.SetDamage(weapon.GetAttribute("extra shot damage", 100))
	firebullets.SetShots(weapon.GetAttribute("extra shot bullet", 8))
